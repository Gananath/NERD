...
def select_candidates(X,ts,char_idx,chars,actor_critic,net,total=100):
    SavedAction = namedtuple('SavedAction', ['smi','rew','fit','log_p1','log_p2', 'value'])
    action_taken=[]
    saved_actions =[]
    for i  in range(int(total/2)):
        ...
        for i, j,k,l in zip(smiles_reward_fit,log_probs1,log_probs2,state_value):
            ...
            rew = i[1]
            fit = i[2]
            saved_actions.append(SavedAction(sm,rew,fit,j,k,l))
...
def get_reward_fitness(state,X,ts,char_idx,chars,net):
    ...
    arr = np.hstack((reward.reshape(len(reward),1),fitness.reshape(len(fitness),1)))
    try:
