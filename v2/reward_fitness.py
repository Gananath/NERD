...
if not os.path.exists('data_v2.npy'):
    ...
    fake_data = np.vstack((fake_data,-np.ones(len(fake_data)))).T
...
y = df.Labels.values.astype(float).reshape(len(df.Labels),1)


# Neural network model
net = Net(seq_len=X_dash.shape[1],char_len = X_dash.shape[2],output_dim = [1,1])
...
for e in pbar:
    ...
    '''
    # Validation
    y_pred = net(T.from_numpy(X_test).float(),None).detach().numpy()
    ...
    if loss.item() < prev_loss:
        T.save(net.state_dict(), 'model_v2.pth')
        #score = acc
        prev_loss = loss.item()
