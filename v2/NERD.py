...
net = Net(seq_len=125, char_len=25,output_dim=[1,1])
net.load_state_dict(T.load('model_v2.pth'))

# Load data
df =  np.load('data_v2.npy')
...
for i in range(epochs):
    ...
    total_reward = 0
    smile_reward_fit = []
    for (smi,R,log_p1,log_p2,value) in candidates:
        R =  float(R)
    ...
    # Selecting new seeds 
    df = pd.DataFrame(smile_reward_fit, columns =['SMILES','Rew_Fit'])
    df[["Rew_Fit"]] = df[["Rew_Fit"]].apply(pd.to_numeric)
    df.sort_values(['Rew_Fit'], ascending=[False],inplace = True)
    if i%10 == 0:
